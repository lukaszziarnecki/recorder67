...
* 🎙️ **Czysta Lista Mikrofonów i Priorytetyzacja WASAPI:** Automatyczna deduplikacja urządzeń audio na liście wyboru (koniec powielania tego samego fizycznego mikrofonu przez MME, DirectSound i WASAPI), ukrywanie zbędnych aliasów systemowych i domyślny wybór WASAPI z automatycznym zapasowym przełączaniem (fallback).
* 🛡️ **Samonaprawiający się Watchdog Strumieni Audio:** Wielostopniowa detekcja zawieszenia buforów sprzętowych mikrofonu i pętli zwrotnej (np. po wielogodzinnych maratonach, konfliktach z dyktowaniem w przeglądarce czy uśpieniu urządzenia bezprzewodowego) z automatycznym restartem i odtworzeniem strumienia w locie bez konieczności ponownego uruchamiania aplikacji.
